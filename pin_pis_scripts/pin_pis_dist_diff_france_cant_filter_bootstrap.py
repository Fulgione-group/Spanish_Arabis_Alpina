...
output_dist=open("pin_pis_dist_diff_france_spain_filter_bootstrap.txt","w")
output=open("pin_pis_stats_diff_france_spain_filter_bootstrap.tsv","w")
...
for gene in genes:
    ...
    df_france_gene["pi_france"] = pd.to_numeric(df_france_gene["pi_france"],errors="coerce")
    df_cantabria_gene["pi_cant"] = pd.to_numeric(df_cantabria_gene["pi_cant"],errors="coerce")
    if len(df_france_gene[df_france_gene["pi_france"]!=0])>2 and len(df_cantabria_gene[df_cantabria_gene["pi_cant"]!=0])>2:
        ...
        df_f_c_gene_n["pi_france"]=pd.to_numeric(list(df_f_c_gene_n.pi_france), errors='coerce').astype(float)
        df_f_c_gene_n["pi_cant"]=pd.to_numeric(list(df_f_c_gene_n.pi_cant), errors='coerce').astype(float)
        df_f_c_gene_s["pi_france"]=pd.to_numeric(list(df_f_c_gene_s.pi_france), errors='coerce').astype(float)
        df_f_c_gene_s["pi_cant"]=pd.to_numeric(list(df_f_c_gene_s.pi_cant), errors='coerce').astype(float)
        pin_france_np=df_f_c_gene_n["pi_france"].to_numpy()
        pis_france_np=df_f_c_gene_s["pi_france"].to_numpy()
        pin_cant_np=df_f_c_gene_n["pi_cant"].to_numpy()
        pis_cant_np=df_f_c_gene_s["pi_cant"].to_numpy()
        print(pin_france_np.shape, pis_france_np.shape, pin_cant_np.shape, pis_cant_np.shape)
        if pin_france_np.shape[0]>=2 and pis_france_np.shape[0]>=2 and pin_cant_np.shape[0]>=2 and pis_cant_np.shape[0]>=2:
            ...
            output_dist=open("pin_pis_dist_diff_france_spain_filter_bootstrap.txt","a")
