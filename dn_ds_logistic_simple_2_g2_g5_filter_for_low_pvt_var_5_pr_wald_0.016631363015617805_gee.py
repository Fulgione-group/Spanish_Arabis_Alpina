...
df_log=pd.read_csv("output_log_reg_df_substitution.tsv",sep="\t")
output=open("statistics_per_gene_substitution_g2_g5_filter_low_pvt_var_5_pr_wald0.016631363015617805_gee.tsv","w")
output.write("gene"+"\t"+"OR"+"\t"+"coeff"+"\t"+"pvalues"+"\t"+"bse"+"\t"+"wald_p"+"\n")
genes=df_log["gene"].unique()
R = np.array([[0, 1]])  # select coefficient 1 (second element)
q = np.array([0.016631363015617805])
for gene in genes:
	df_log_gene=df_log[df_log["gene"]==gene]
	if len(df_log_gene[(df_log_gene["set"]=="france") & (df_log_gene["ns"]=="n") ])>2 and len(df_log_gene[(df_log_gene["set"]=="france") & (df_log_gene["ns"]=="s") ])>2 and len(df_log_gene[(df_log_gene["set"]=="cantabria") & (df_log_gene["ns"]=="n") ])>2 and len(df_log_gene[(df_log_gene["set"]=="cantabria") & (df_log_gene["ns"]=="s") ])>2 and len(df_log_gene[df_log_gene["set"]=="france"])>5 and len(df_log_gene[df_log_gene["set"]=="cantabria"])>5:
		df_log_gene.sort_values(by=['pos'])
		if len(df_log_gene["pos"].value_counts()[df_log_gene["pos"].value_counts()==1])>5:
		        syn=[]
